...
public:
    ...
    void TestCalculateSheetHR()
    {
        ...
        std::ifstream gradijk("/tmp/ravc486/testoutput/TestLaplace_rat_8_8_1_longi/rat_8_8_1_grad_longi.txt");
        if (!gradijk)
        {
            cout << "There was a problem opening laplace gradient for reading " << endl;
        }

        std::ifstream gradNormalijk("/tmp/ravc486/testoutput/TestLaplace_rat_8_8_1_circum/rat_8_8_1_grad_circum.txt");
        if (!gradNormalijk)
        {
            cout << "There was a problem opening laplace gradient normal for reading " << endl;
        }

        OutputFileHandler output_file_handler("TestLaplace_rat_8_8_1_ortho_v2");
        out_stream p_file = output_file_handler.OpenOutputFile("rat_8_8_1.1.ortho");
        double x, y, z;
        Vector3d fibre;
        Vector3d sheet;
        Vector3d normal;
        ...
        p_file->close();
        VtkMeshWriter<3u, 3u> mesh_writer("TestLaplace_rat_8_8_1_ortho_v2", "mesh", false);
